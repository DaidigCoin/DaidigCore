...
const char * const BITCOIN_CONF_FILENAME = "daidig.conf";
const char * const BITCOIN_PID_FILENAME = "daidigd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "daidig" is a composite category enabling all Daidig-related debug output
                if(ptrCategory->count(std::string("daidig"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Daidig
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Daidig
    // Mac: ~/Library/Application Support/Daidig
    // Unix: ~/.daidig
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Daidig";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Daidig";
#else
    // Unix
    return pathRet / ".daidig";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty daidig.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override daidig.conf
