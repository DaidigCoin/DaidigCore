...
# daidig_hash
RUN git clone https://github.com/daidigpay/daidig_hash
RUN cd daidig_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} daidig
RUN useradd -u ${USER_ID} -g daidig -s /bin/bash -m -d /daidig daidig
...
RUN mkdir /daidig-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /daidig-src && \
