...
// which are part of the Daidig repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="daidig-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /daidig-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "daidigpay-daidig/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from daidigpay-daidig/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'daidigpay-daidig/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from daidigpay-daidig/develop"
            }
          }

          if (hasCache) {
            sh "cd /daidig-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /daidig-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /daidig-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /daidig-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /daidig-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /daidig-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /daidig-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
